// Init single office
function init_office(id) {
    load_small_table_item(id, '#office', 'officeid', 'offices/get_office_data_ajax', '.table-offices');
}


// Validates office add/edit form
function validate_office_form(selector) {

    selector = typeof (selector) == 'undefined' ? '#office-form' : selector;
    ...
    $("body").find('input[name="number"]').rules('add', {
        remote: {
            url: admin_url + "offices/validate_office_number",
            type: 'post',
            data: {
                number: function () {
                ...
                date: function () {
                    return $('body').find('.office input[name="date"]').val();
                },
            }
        },
        messages: {
            remote: app.lang.office_number_exists,
        }
    });
...
function get_office_item_preview_values() {
    var response = {};
...
function add_office_item_to_table(data, itemid){

  // If not custom data passed get from the preview
  data = typeof (data) == 'undefined' || data == 'undefined' ? get_office_item_preview_values() : data;
...
// From office table mark as
function office_mark_as(status_id, office_id) {
    var data = {};
    data.status = status_id;
    data.officeid = office_id;
    $.post(admin_url + 'offices/update_office_status', data).done(function (response) {
        //table_offices.DataTable().ajax.reload(null, false);
        reload_offices_tables();
    });
}

// Reload all offices possible table where the table data needs to be refreshed after an action is performed on task.
function reload_offices_tables() {
    var av_offices_tables = ['.table-offices', '.table-rel-offices'];
    $.each(av_offices_tables, function (i, selector) {
